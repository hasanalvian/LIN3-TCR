...
Bots=[mid,Amid,Bmid]
admin=["ude93c90c545f14b9947336167615b098"]
wait = {
    'contact':False,
    'autoJoin':True,
        ...
        if op.type == 25:
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
                ...
                ##cl.sendMessage(msg)
            elif msg.text in ["O","Link on"]:
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                    X.preventJoinByTicket = False
                    cl.updateGroup(X)
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"Wes tak buka")
                    else:
            ...
            elif msg.text in ["C","Link off"]:
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                    X.preventJoinByTicket = True
                    cl.updateGroup(X)
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"Yo wes tak tutup maneh")
                    else:
...
#-----------------------------------------------
            ...
            elif msg.text in ["All join","In","Rene"]:
...
#-----------------------------------------------
            elif msg.text in ["Bye all","Muleh","Out"]:
                if msg.toType == 2:
                    ginfo = cl.getGroup(msg.to)
                    try:
...
#-----------------------------------------------
            elif msg.text in ["Respon","On","respon"]:
                ki.sendText(msg.to,"Ono")
                kk.sendText(msg.to,"Opo")
 #-----------------------------------------------               
            elif msg.text in ["M"]:
                cl.sendText(msg.to,"/mulai")
            elif msg.text in ["N"]:
                cl.sendText(msg.to,"/nyerah")
#-----------------------------------------------

            elif msg.text in ["Sp","Speed","speed"]:
                start = time.time()
                cl.sendText(msg.to, "Bentar Bosss...")
